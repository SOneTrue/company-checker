...
from tgbot.models.users import update_info_user, add_user
...
async def user_fuel(message: Message, state: FSMContext):
    if message.text == '/start':
        ...
        await state.update_data(photo_one=message.photo[0].file_id)
...
async def user_auto_front(message: Message, state: FSMContext):
    if message.text == '/start':
        ...
        await state.update_data(photo_two=message.photo[0].file_id)
...
async def user_auto_back(message: Message, state: FSMContext):
    if message.text == '/start':
        ...
        await state.update_data(photo_three=message.photo[0].file_id)
...
async def user_auto_left(message: Message, state: FSMContext):
    if message.text == '/start':
        ...
        await state.update_data(photo_four=message.photo[0].file_id)
...
async def user_fuel_back(message: Message, state: FSMContext):
    if message.text:
        ...
        await state.update_data(photo_six=message.photo[0].file_id)
        await message.answer(
            f'Оставьте комментарий, если были выявлены неисправности в течении дня или нажмите кнопку "Нет".',
            reply_markup=answer_day)
        user_data = await state.get_data()
        await add_user(telegram_id=message.from_user.id, username=None, fname=None, lname=None, real_name=user_data['real_name'],
